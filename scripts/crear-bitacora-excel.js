...
const datosLog = [
  ...
  ['__HOY__', '__AHORA__', 'To-Do: diaria Hecha al vencer', 'Una tarea con recurrencia diaria no se puede marcar Hecha antes de su vencimiento (control en bandeja y en todo_cambiar_estado). SQL supabase_todo_diaria_hecha_al_vencer.sql.', 'Ajuste'],
  ['__HOY__', '__AHORA__', 'Despliegue v1.34', 'To-Do: las diarias no se marcan Hecha antes del vencimiento. everfit-release.json v1.34. Push a main y vercel --prod.', 'Despliegue'],
  ['__HOY__', '__AHORA__', 'To-Do: diaria Hecha el mismo día', 'Diaria: se puede marcar Hecha en cualquier hora del día de vencimiento (Argentina); solo se bloquea si el vencimiento es un día posterior a hoy. Bandeja + todo_cambiar_estado. SQL supabase_todo_diaria_hecha_mismo_dia.sql.', 'Ajuste'],
  ['__HOY__', '__AHORA__', 'To-Do: card Hechas hoy', 'La card Hechas hoy ya no sale del listado Abiertas (quedaba en 0). RPC todo_list_hechas_hoy por día calendario AR, sin recorte por hora. Respeta sede, prioridad y búsqueda.', 'Corrección'],
  ['__HOY__', '__AHORA__', 'Despliegue v1.35', 'To-Do: diaria Hecha el mismo día (sin hora límite); card Hechas hoy independiente del filtro Abiertas. everfit-release.json v1.35. Push a main y vercel --prod.', 'Despliegue'],
  ['__HOY__', '__AHORA__', 'To-Do: modal responsive', 'Modales nueva/editar tarea: cuerpo con scroll, footer fijo con botones visibles en PC y móvil (safe-area, 768px).', 'Ajuste'],
  ['__HOY__', '__AHORA__', 'To-Do: columna Estado por', 'Bandeja, matriz y Excel muestran quién cambió el estado por última vez. Columna todo_instancia.estado_modificado_por; todo_cambiar_estado y todo_list_inbox. SQL supabase_todo_estado_modificado_por.sql.', 'Feature'],
  ['__HOY__', '__AHORA__', 'Permiso ver_dashboard', 'Menú Home (Dashboard) configurable por rol en Seguridad, igual que Asistencia (ver_asistencia). Vista inicial según permisos. SQL supabase_menu_ver_dashboard.sql.', 'Feature'],
  ['__HOY__', '__AHORA__', 'Mi perfil: nombre de usuario', 'Columna nombre_usuario en user_profiles; modal Mi perfil en barra superior; bandeja To-Do (Responsable y Usuario) y combos usan nombre en lugar de email. RPC get_my_profile, set_my_nombre_usuario, user_profile_label. SQL supabase_user_profiles_nombre_usuario.sql.', 'Feature'],
  ['__HOY__', '__AHORA__', 'Seguridad: editar nombre usuario', 'Tabla Usuarios con email + campo Nombre para mostrar; Guardar persiste rol y nombre (RPC set_user_nombre_usuario). Columna bandeja To-Do renombrada a Usuario. SQL supabase_set_user_nombre_usuario_admin.sql.', 'Feature'],
